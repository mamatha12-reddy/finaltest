...
function AddToCart(props){
    let productarr=props.productarr
    let count=props.count
    const removeFromCart=(product)=>{
        console.log("the removing product",product)
       // let username=localStorage.getItem("username")

        axios.post(`/user/removefromcart/${localStorage.getItem('username')}`,product)
        .then(res=>{
            let resObj=res.data
   ...
   <table className="table border">
        ...
        <tbody>
        {
            productarr &&    productarr.map(ele=>{
                    return(
                    <tr>
                        <td className="mt-4">{ele.productname}</td>
                        <td className="mt-4">{ele.details}</td>
                        <td className="mt-4">{ele.rating}</td>
                        <td className="mt-4"><img src={ele.pfofileImge} width="100px" height="100px"/></td>
                        <td><button className="btn btn-danger text-white" onClick={()=>removeFromCart(ele)}>Remove</button></td>
                    </tr>
